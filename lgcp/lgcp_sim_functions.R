...
palm_lgcp_sim_study<-function(nsim,mu,sig2,phi,wsize,R,nboot=100,empirical=TRUE){
...
disc_palm_lgcp_sim_study<-function(nsim,mu,sig2,phi,wsize,R,nboot=100,empirical=TRUE){
  S_all<-readRDS(paste0('sim_output/lgcp/sim_data_',round(mu),'_',wsize,'.rds'))
  if(empirical){rho_sd<-1} else{rho_sd<-100}
  ...
  saveRDS(all_out,paste0('sim_output/lgcp/disc_palm_output_',R,'_',round(rho_sd),'_',round(mu),'_',wsize,'.rds'))
...
full_lgcp_sim_study<-function(nsim,mu,sig2,phi,wsize){
  ...
  saveRDS(all_out,paste0('sim_output/lgcp/full_output_',R,'_',round(rho_sd),'_',round(mu),'_',wsize,'.rds'))
}


### ONE OFF MODEL FIT WRAPPER
fit_palm_lgcp<-function(S,R,wsize,empirical=TRUE,nboot=100){
  model<-stan_model('lgcp/lgcp_palm_disc.stan')
  if(empirical){rho_sd<-1} else{rho_sd<-100}
  # ORGANIZE DATA
  data<-data_clean(S=S,R=R,wsize)
